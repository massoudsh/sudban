...
| **Wisdom Engine** | `wisdomEngine.ts` | خروجی پنج مورد بالا (ترکیبی) | `WisdomReport` (`insights[]` اولویت‌بندی‌شده + `topRecommendation`) — جزئیات: [[Wisdom Engine|Wisdom-Engine]] |
...
- `checkRisks` حداکثر ۳ نوع هشدار همزمان می‌تواند برگرداند (loss-making با low-margin هم‌زمان رخ
  نمی‌دهد چون `else if`). آستانه‌ها اکید هستند (`>` و `<`)، پس مقدار دقیقاً روی مرز هشدار تولید
  نمی‌کند.
