# بک‌لاگ Issue — آماده برای ساخت در گیت‌هاب

> این فایل به‌صورت محلی نوشته شده چون کانتینر فعلی به گیت‌هاب دسترسی نوشتن ندارد (بدون `gh` و
> بدون توکن معتبر). هر بخش دقیقاً در قالب یک GitHub Issue (عنوان + برچسب‌ها + بدنه) نوشته شده تا
> با در اختیار داشتن یک Personal Access Token جدید، مستقیماً با `gh issue create` یا API ساخته شود.

## نزدیک‌مدت (Near-term — توسعه بعدی مستقیم روی MVP فعلی)

### 1. تست واحد برای پنج+۱ موتور منطق قیمت‌گذاری
...
### 2. اعتبارسنجی ورودی ساخت‌یافته (به‌جای بررسی دستی در route)
...
### 3. احراز هویت و مجوز سطح فروشنده (Auth)
...
### 4. Import دسته‌ای CSV برای CostProfile / SalesRecord / CompetitorPrice
labels: `enhancement`, `priority:medium`
```
...
### 5. Rate limiting و محافظت پایه API
...
### 6. کانال هشدار خارج از API (تلگرام/ایمیل/پیامک)
labels: `enhancement`, `priority:medium`
```
...
### 7. موتور پیشنهاد قیمت مبتنی بر یادگیری ماشین (v2)
labels: `future`, `ml`
```
...
### 8. اسکرپینگ خودکار قیمت رقبا (دیجی‌کالا/باسلام/ترب)
...
### 9. اتصال مستقیم به درگاه فروش برای اعمال خودکار قیمت (auto price-push)
labels: `future`, `integration`
```
...
### 10. سیگنال نرخ ارز/تورم به‌عنوان ورودی خودکار CostProfile
labels: `future`, `data`
```
...
### 12. چندمستأجری با سطوح دسترسی سازمانی
...
### 13. مدل قیمت‌گذاری اشتراکی + متری usage-based
